...
public class ReactionFingerprint extends Fingerprint {
    ...
    public ReactionFingerprint(ReactionDescription reactionDescription, byte[] postingDigest) {
        super(0);
        ownerName = reactionDescription.getOwnerName();
        postingFingerprint.setDigest(postingDigest);
        negative = reactionDescription.isNegative();
        emoji = reactionDescription.getEmoji();
    }
