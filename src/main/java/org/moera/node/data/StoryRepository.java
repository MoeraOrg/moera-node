...
public interface StoryRepository extends JpaRepository<Story, UUID>, QuerydslPredicateExecutor<Story> {
    ...
    int countSubsByTypeAndEntryId(UUID nodeId, StoryType storyType, UUID entryId);

    @Query(
        "select s from Story s left join fetch s.substories"
        + " where s.nodeId = ?1 and s.feedName = ?2 and s.storyType = ?3 and s.entry.id = ?4"
    ...
    @Query(
        "select s from Story s"
        + " left join fetch s.remoteAvatarMediaFile"
        + " left join fetch s.remoteOwnerAvatarMediaFile"
    ...
    void deleteByRemotePostingAndCommentId(
        UUID nodeId, String feedName, StoryType storyType,
    ...
    void deleteByRemotePostingId(
        UUID nodeId, String feedName, StoryType storyType, String remoteNodeName, String remotePostingId
    );
    ...
    List<Story> findByRemotePostingId(
        UUID nodeId, String feedName, StoryType storyType, String remoteNodeName, String remotePostingId
    );
    ...
    List<Story> findFullByRemotePostingId(
        UUID nodeId, String feedName, StoryType storyType, String remoteNodeName, String remotePostingId
    );
    ...
    List<Story> findFullByRemotePostingAndCommentId(
        UUID nodeId, String feedName, StoryType storyType,
        String remoteNodeName, String remotePostingId, String remoteCommentId
    );
    ...
    @Query(
        "select s from Story s left join fetch s.substories"
        + " where s.nodeId = ?1 and s.feedName = ?2 and s.storyType = ?3 and s.remoteNodeName = ?4"
        + " and s.remotePostingId = ?5 and s.remoteRepliedToId = ?6"
        + " order by s.moment desc"
    )
    List<Story> findFullByRemotePostingAndRepliedToId(
        UUID nodeId, String feedName, StoryType storyType,
        String remoteNodeName, String remotePostingId, String remoteRepliedToId
    );

    @Query(
        "select s from Story s left join fetch s.parent"
        + " where s.nodeId = ?1 and s.parent is not null and s.storyType = ?2 and s.remoteNodeName = ?3"
        + " and s.remotePostingId = ?4 and s.remoteCommentId = ?5"
        + " order by s.moment desc"
    )
    List<Story> findSubsByRemotePostingAndCommentId(
        UUID nodeId, StoryType storyType, String remoteNodeName, String remotePostingId, String remoteCommentId
    );
