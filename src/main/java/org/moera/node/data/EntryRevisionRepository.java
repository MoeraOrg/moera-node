...
public interface EntryRevisionRepository extends JpaRepository<EntryRevision, UUID> {
    ...
    @Query(
        "select r.entry.id from EntryRevision r where r.entry.nodeId = ?1 and r.entry.entryType = ?2"
    ...
    @Modifying
    @Query(
        "update Entry e set e.totalRevisions = (select count(*) from EntryRevision r where r.entry.id = e.id)"
        + " where e.id = ?1"
    )
    ...
    @Query(
        ...
        + " where exists("
        + "select ea from EntryAttachment ea where ea.entryRevision.id = r.id and ea.mediaFileOwner.id = ?1"
        + ")"
    )
    ...
    @Query(
        "update EntryRevision r set r.attachmentsCache = null"
        + " where r.attachmentsCache is not null and exists("
        + "select ea from EntryAttachment ea"
        + " where ea.entryRevision.id = r.id and (ea.mediaFileOwner.mediaFile.id = ?1 or exists("
        + "select mfp from MediaFilePreview mfp"
        + " where mfp.originalMediaFile = ea.mediaFileOwner.mediaFile and mfp.mediaFile.id = ?1"
        + "))"
        + ")"
    )
    void clearAttachmentsCacheByMediaFile(String mediaFileId);
    ...
    @Query(
        ...
        + "select ea from EntryAttachment ea"
        + " where ea.entryRevision = r and ea.remoteMediaFile.nodeName = ?2 and ea.remoteMediaFile.mediaId = ?3"
        + ")"
    )
