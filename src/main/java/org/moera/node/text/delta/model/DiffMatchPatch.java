...
public class DiffMatchPatch {
     ...
     * allows the 'checklines' of diffMain() to be optional. Most of the time checklines is wanted,
     * so default to true.
     *
            ...
            switch (token.charAt(0)) {
                ...
                case '=':
                    ...
                    try {
                        int prev = pointer;
                        pointer += n;
                        text = text1.substring(prev, pointer);
     ...
     * @param diffs Array of Diff objects for text1 to text2.
     * @return LinkedList of Patch objects.
     * @deprecated Prefer patchMake(String text1, LinkedList&lt;{@link Diff}&gt; diffs).
     */
    @Deprecated
                            ...
                            if (aDiff.operation != Operation.EQUAL) {
                                ...
                                if (aDiff.operation == Operation.INSERT) {
                                    ...
                                    text =
                                            text.substring(0, startLoc + index2)
                                                    + text.substring(
                                                    startLoc + diffxIndex(diffs, index1 + aDiff.text.length()));
                                }
                            }
                            if (aDiff.operation != Operation.DELETE) {
     ...
     * algorithm. Intended to be called only from within patchApply.
     *
            ...
            while (!bigpatch.diffs.isEmpty()) {
                ...
                while (!bigpatch.diffs.isEmpty() && patch.length1 < patchSize - patchMargin) {
                    ...
                    if (diffType == Operation.INSERT) {
                        ...
                        diffText =
                                diffText.substring(
                                        0, Math.min(diffText.length(), patchSize - patch.length1 - patchMargin));
                        patch.length1 += diffText.length();
                        start1 += diffText.length();
                        ...
                        if (diffText.equals(bigpatch.diffs.getFirst().text)) {
                            ...
                            bigpatch.diffs.getFirst().text =
                                    bigpatch.diffs.getFirst().text.substring(diffText.length());
                        }
                    }
                }
     ...
     * @return Linked List of Diff objects.
     */
    private LinkedList<Diff> diffMain(
     ...
     * @return Linked List of Diff objects.
     */
    private LinkedList<Diff> diffCompute(
            String text1, String text2, boolean checklines, long deadline) {
        LinkedList<Diff> diffs = new LinkedList<Diff>();
     ...
     */
    private String diffLinesToCharsMunge(
