...
public class ProfileController {
    ...
    public ProfileInfo put(@RequestBody ProfileAttributes profileAttributes) {
        ...
        return ProfileInfoUtil.build(requestContext, true, directServeOperations);
    }

    @PostMapping("/email/verify")
    @RateLimit(limit = 5, period = 900)
    @Admin(Scope.UPDATE_PROFILE)
    public Result emailVerify() {
        log.info("POST /profile/email/verify");

        String email = requestContext.getOptions().getString("profile.email");
        if (ObjectUtils.isEmpty(email)) {
            throw new OperationFailure("profile.email-not-set");
        }

        String token = emailVerificationOperations.createVerification(requestContext.nodeId(), email);
        requestContext.send(new EmailVerificationLiberin(requestContext.nodeName(), token));
