...
public class AllRemoteProfilesSubscriptionTask extends Task {
    ...
    private static Logger log = LoggerFactory.getLogger(AllRemoteProfilesSubscriptionTask.class);
    ...
    private void subscribe(String targetNodeName) throws Throwable {
        String targetFullName = nodeApi.whoAmI(targetNodeName).getFullName();
        inTransaction(() -> {
            subscriberRepository.updateRemoteFullName(nodeId, targetNodeName, targetFullName);
            subscriptionRepository.updateRemoteFullName(nodeId, targetNodeName, targetFullName);
            contactRepository.updateRemoteFullName(nodeId, targetNodeName, targetFullName);
            return null;
        });
